...
from sceptre.resolvers.file import File
...
class TestFileResolver(unittest.TestCase):
    ...
    def test_resolving_with_valid_url_json_file(self):
        url_ref = "https://raw.githubusercontent.com/Sceptre/sceptre/master/integration-tests/sceptre-project/templates/valid_template.json"  # noqa
        self.file_resolver.argument = url_ref
        result = self.file_resolver.resolve()
        result_json = json.dumps(result)
        expected = '{"Resources": {"WaitConditionHandle": {"Type": "AWS::CloudFormation::WaitConditionHandle", "Properties": {}}}}'  # noqa
        assert expected == result_json

    def test_resolving_with_valid_url_yaml_file(self):
        url_ref = "https://raw.githubusercontent.com/Sceptre/sceptre/master/integration-tests/sceptre-project/templates/valid_template.yaml"  # noqa
        self.file_resolver.argument = url_ref
        result = self.file_resolver.resolve()
        result_json = json.dumps(result)
        expected = '{"Resources": {"WaitConditionHandle": {"Type": "AWS::CloudFormation::WaitConditionHandle", "Properties": null}}}'  # noqa
        assert expected == result_json

    def test_resolving_with_invalid_url_file(self):
        with pytest.raises(requests.exceptions.RequestException):
            self.file_resolver.argument = "https://raw.githubusercontent.com/Sceptre/sceptre/master/non_existing_file"  # noqa
            self.file_resolver.resolve()
